...
`
var preCommitAppendScriptTemplate = `
# Below script generated by env-sample-sync https://github.com/acaloiaro/env-sample-sync
ARGS=({{ .Args }})
...
func installHook(gitDirPath string) (err error) {
	...
	if os.IsNotExist(err) {
		...
		tmpl, err := template.New("pre-commit-script").Parse(preCommitAppendScriptTemplate)
		if err != nil {
			return err
		}
		...
		//open the file with append flag set true
		existingPreCommitHook, err := os.OpenFile(preCommitHookScriptPath, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
		if err != nil {
			return err
		}
		defer existingPreCommitHook.Close()

		//write the template to the file
		_, err = buff.WriteTo(existingPreCommitHook)
		if err != nil {
			return err
		}
	}
